...
def shortest_path(source, target):
    ...
    while True:
        ...
        # Check if source name and target name are the same
        if source == target:
            return []
        ...
        # Remove the actor to be explored from frontier while increase the counter by 1 and add to the explored node
        node = frontier.remove()
        no_of_node_explored = no_of_node_explored + 1
        explored_node.add(node.state)

        # Get all the neighbors for the actor
        neighbors = neighbors_for_person(node.state)

        # Explore all the neighbors
        for movie_ids, actor_ids in neighbors:
